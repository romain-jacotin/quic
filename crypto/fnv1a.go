...
func ComputeHashFNV1A_64(data []byte) uint64 {
...
func ComputeAeadHashFNV1A_128(aad, text []byte) (high, low uint64) {
	high, low = ComputeHashFNV1A_128(aad)
	high, low = IncrementalHashFNV1A_128(high, low, text)
	return
}

func IncrementalHashFNV1A_128(high_prev, low_prev uint64, data []byte) (high, low uint64) {
	var val [4]uint64
	...
	val[0] = low_prev & 0xffffffff
	val[1] = low_prev >> 32
	val[2] = high_prev & 0xffffffff
	val[3] = high_prev >> 32
	for _, v := range data {
		// xor the bottom with the current octet
		val[0] ^= uint64(v)
...
func ComputeHashFNV1A_128(data []byte) (high, low uint64) {
